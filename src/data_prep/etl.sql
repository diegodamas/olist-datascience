...
CREATE TABLE PRE_ABT_{stage}_CHURN AS
        ...
        SELECT t1.seller_id,
                ...
                COUNT(DISTINCT DATE_FORMAT(t1.dt_venda, '%%%%m')) / 6.0 AS prop_ativacao
        ...
        FROM(
                ...
                AND t1.order_purchase_timestamp >= DATE_SUB('{date}', INTERVAL 6 month)
                AND t1.order_status = 'delivered'
        ) AS t1

        GROUP BY t1.seller_id

        HAVING MAX(t1.dt_venda) >=  DATE_SUB('{date}', INTERVAL 3 month)
;